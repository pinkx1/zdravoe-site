<?php
// Используем тот же шаблон, что и home.php
get_header();
?>
			...
			<a href="<?php the_permalink(); ?>" class="group block bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
				<div class="aspect-[3/4] overflow-hidden bg-gray-100">
					<?php if (has_post_thumbnail()) {
						the_post_thumbnail('book_card', ['class' => 'w-full h-full object-cover group-hover:scale-105 transition-transform duration-300']);
					} else {
						...
						echo '<img src="' . esc_url($ph) . '" alt="' . esc_attr(get_the_title()) . '" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" decoding="async" />';
					} ?>
				</div>
				<div class="p-5">
      ...
      while ($articles->have_posts()) : $articles->the_post(); ?>
			<a href="<?php the_permalink(); ?>" class="block bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300">
					...
					<div class="p-6">
						...
						<p class="text-gray-600 mb-4 line-clamp-2"><?php the_excerpt(); ?></p>
